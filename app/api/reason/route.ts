...
Your evaluation must return a **JSON object** with:
- **confidenceThreshold** (number between 0 and 1):
  - 1.0 = Fully correct and complete
  - 0.5 = Partially correct but missing key details
  - 0.0 = Incorrect or misleading
- **reason** (string): A clear explanation of the rating, highlighting missing or incorrect elements.

Strictly follow Wikipedia’s Five Pillars and its principles of accuracy, neutrality, and reliability.
Respond only with a JSON object and no additional commentary.
`;

  const llm = await new OpenRouterCompletion().createReasonCompletion(
    systemPrompt,
    `The question is:\n${question}\n\nThe answer is:\n${answer}`
  );
